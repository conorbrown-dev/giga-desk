...
- Added downloadable Bash and PowerShell Codex worker installers at `/scripts/install-codex-worker.sh` and `/scripts/install-codex-worker.ps1`. They verify Codex, prompt only for node-scoped OIDC values and approved repository mappings, protect local credential files, build the worker, and install/start a PATH-aware systemd user service or per-user Windows Scheduled Task. The PowerShell script is structurally reviewed but remains unexecuted because PowerShell is unavailable on this Linux host.
