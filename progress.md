...
- Passed API unit tests (25 files, 47 tests), API typecheck, API build, and web build. API lint is blocked by a pre-existing unused `actorId` in `prisma-project.repository.ts`; the new hosting test now complies with lint rules. Railway deployment/HTTP readback remains pending until the pull request is merged.
