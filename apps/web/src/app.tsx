...
export function App({ authentication = testAuthentication }: { authentication?: AuthenticationState }) {
  ...
  return (
    <div className="app-shell"><header className="site-header"><nav className="site-nav" aria-label="Primary navigation"><Link className="brand-link" to="/projects"><img className="brand-mark" src="/images/giga-desk-icon.png" alt="" /><span>Giga Desk<small aria-hidden="true">Control center</small></span></Link><div className="nav-links"><NavLink end className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/projects">Projects</NavLink><NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/agents/connect">Connect agent</NavLink></div></nav></header><main className="workspace"><nav className="top-nav" aria-label="Account controls"><span className="account-avatar" aria-hidden="true">{(authentication.username ?? 'G').slice(0, 1).toUpperCase()}</span><span className="account-name" title={authentication.username ?? 'Signed in'}>{authentication.username ?? 'Signed in'}</span><button className="button button-secondary" type="button" onClick={() => { void authentication.logout(); }}>Sign out</button></nav><Routes>
