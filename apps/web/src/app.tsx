...
import { clearQueuedExecution, fetchExecutionHistory, retryExecution, type ExecutionHistory } from './execution-api.js';
...
function ExecutionDashboard() {
  ...
  return <><header className="page-header"><div><Link to="/projects" className="back-link">← Projects</Link><h1>Work Item Execution</h1><p>Track execution attempts, test results, and deployment history.</p></div></header><StartWorkControls key={reload} workItemId={workItemId} onQueued={() => { setReload((value) => value + 1); }} />{error ? <p className="state-panel" role="alert">{error}</p> : history.length === 0 ? <p className="state-panel">No execution attempts yet.</p> : <><div className="summary-stats"><div className="summary-stat"><span className="summary-stat-label">Total Attempts</span><span className="summary-stat-value">{history.length}</span></div><div className="summary-stat"><span className="summary-stat-label">Success Rate</span><span className="summary-stat-value accent-green">{history.length > 0 ? Math.round((completedExecutions / history.length) * 100) : 0}%</span></div><div className="summary-stat"><span className="summary-stat-label">Failures</span><span className="summary-stat-value accent-red">{failedExecutions}</span></div></div><section aria-label="Execution attempts" className="grid">{history.map((execution) => <article className="card" key={execution.id}><div className="card-header"><h2>{execution.agent.name} · {execution.node.name}</h2><span className={statusClassName(execution.status)}>{execution.status}</span></div><div className="status-row"><div className="status-item"><span className="status-label">Model</span><span className="status-value">{execution.model.displayName} ({execution.model.provider})</span></div><div className="status-item"><span className="status-label">Started</span><span className="status-value">{execution.startedAt ? new Date(execution.startedAt).toLocaleString() : 'Pending'}</span></div></div>{execution.failureReason && <div role="alert" className="failure-notice">{execution.failureReason}</div>}{execution.status === 'Queued' && <button type="button" className="button-secondary" onClick={() => { void clearQueuedExecution(workItemId, execution.id).then(() => { setReload((value) => value + 1); }).catch((reason: unknown) => { setError(reason instanceof Error ? reason.message : 'Unable to clear the queued execution.'); }); }}>Clear queued execution</button>}{['Failed', 'Cancelled'].includes(execution.status) && <button type="button" className="button-secondary" onClick={() => { void retryExecution(workItemId, execution.id).then(() => { setReload((value) => value + 1); }).catch((reason: unknown) => { setError(reason instanceof Error ? reason.message : 'Unable to retry the execution.'); }); }}>Retry execution</button>}<h3 className="section-label">Evidence</h3><ul className="evidence-list">{execution.tests.map((test) => <li key={`${test.type}-${test.createdAt}`}><span className="evidence-type">{test.type}</span><span className={test.result === 'Passed' ? 'evidence-passed' : 'evidence-failed'}> · {test.result}</span></li>)}</ul><h3 className="section-label">Deployments</h3>{execution.deployments.length > 0 ? execution.deployments.map((deployment) => <div key={`${deployment.environment}-${deployment.startedAt}`} className="deployment-row"><span className="deployment-env">{deployment.environment}</span><span className={statusClassName(deployment.status)}>{deployment.status}</span></div>) : <p className="muted-copy">No deployments yet</p>}</article>)}</section></>}</>;
}
