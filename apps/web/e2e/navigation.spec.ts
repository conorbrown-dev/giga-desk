...
test('navigates from projects to a work item execution dashboard', async ({ page }) => {
  ...
  await accountControls.getByRole('button', { name: 'Open account menu for test-user' }).click();
...
test('walks through Codex agent setup in the authenticated app', async ({ page }) => {
  ...
  await page.setViewportSize({ width: 1920, height: 1080 });
  await page.screenshot({ path: 'test-results/visual-review/codex-connect-wide.png', fullPage: true });
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.screenshot({ path: 'test-results/visual-review/codex-connect-laptop.png', fullPage: true });
  await page.setViewportSize({ width: 1024, height: 768 });
  await page.screenshot({ path: 'test-results/visual-review/codex-connect-tablet.png', fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
...
test('shows only OpenCode setup when OpenCode is selected', async ({ page }) => {
  ...
  await expect(page.getByText(/registers through the authenticated API/)).toBeVisible();
  await expect(page.getByRole('link', { name: 'Download Bash installer' })).toHaveAttribute('href', '/scripts/install-opencode-worker.sh');
  await expect(page.getByRole('link', { name: 'PowerShell installer' })).toHaveAttribute('href', '/scripts/install-opencode-worker.ps1');
